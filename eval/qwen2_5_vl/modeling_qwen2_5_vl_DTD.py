...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def reverse_pixel_values_videos(self, pixel_values_videos: torch.Tensor, 
                                    grid_t: int, grid_h: int, grid_w: int, temporal_patch_size: int,
                                    merge_size: int, channel: int, patch_size: int, debug: bool = False) -> torch.Tensor:
        """
        Reverse the pixel values of the videos.
        """
        unpatch_pixel_values_videos = pixel_values_videos.view(
            grid_t, 
            grid_h // merge_size, 
            grid_w // merge_size, 
            merge_size, 
        ...
        unpatch_pixel_values_videos = unpatch_pixel_values_videos.permute(0, 6, 1, 2, 5, 3, 4, 7, 8)   # t t_p h_m w_m c m0 m1 p0 p1
        unpatch_pixel_values_videos = rearrange(unpatch_pixel_values_videos, 't t_p h_m w_m c m0 m1 p0 p1 -> (t t_p) c (h_m m0 p0) (w_m m1 p1)')
        
        # if debug:
        #     import matplotlib.pyplot as plt
        #     unpatch_img_vis = unpatch_pixel_values_videos[0].permute(1, 2, 0).float().cpu().numpy()
        #     unpatch_img_vis = (unpatch_img_vis - unpatch_img_vis.min(axis=(0, 1, 2))) / (unpatch_img_vis.max(axis=(0, 1, 2)) - unpatch_img_vis.min(axis=(0, 1, 2)))
        #     plt.imsave(f'unpatch_img_vis.png', unpatch_img_vis)

        return unpatch_pixel_values_videos
    ...
    def token_drop(
        ...
        #NOTE: visualize the pixel values of the videos
        if debug:
            pixel_values_videos_vis = self.reverse_pixel_values_videos(pixel_values_videos, grid_t,
                                                                        grid_h, grid_w, temporal_patch_size,
                                                                        merge_size, channel, patch_size, debug)
        ...
        for i in range(batch_size):
            ...
            if debug:
                visual_token_indices_list = list(visual_token_indices)
                keep_mask_vis = ~keep_mask.clone()[visual_token_indices_list].view(-1, (grid_h // merge_size), (grid_w // merge_size))
                keep_mask_vis = F.interpolate(keep_mask_vis[:,None,...].float(), size=(grid_h*patch_size, grid_w*patch_size), mode='nearest')
                #NOTE: visualize the mask video with cv2.addweight
                # pixel_values_videos_vis: T 3 H W
                # keep_mask_vis: T//2 1 H W, two frames share one mask
                frames = []
                for k_ in range(keep_mask_vis.shape[0]):
                    keep_mask_vis_i = keep_mask_vis[k_]
                    # Implement visualization using cv2.addWeighted
                    # Two frames share one mask
                    frame_vis_1 = pixel_values_videos_vis[2 * k_].clone()
                    frame_vis_1 = (frame_vis_1 - frame_vis_1.min()) / (frame_vis_1.max() - frame_vis_1.min())
                    frame_vis_2 = pixel_values_videos_vis[2 * k_ + 1].clone()
                    frame_vis_2 = (frame_vis_2 - frame_vis_2.min()) / (frame_vis_2.max() - frame_vis_2.min())
                    mask_vis = torch.zeros_like(frame_vis_1)  # Use pure white mask
                    # Apply mask only where keep_mask_vis_i is True, otherwise use original frame color
                    mask_applied_1 = torch.where(keep_mask_vis_i.expand_as(frame_vis_1).bool(), mask_vis, frame_vis_1)
                    visualized_frame_1 = cv2.addWeighted(frame_vis_1.float().cpu().numpy(), 0.3, mask_applied_1.float().cpu().numpy(), 0.7, 0)
                    frames.append(visualized_frame_1.transpose(1, 2, 0))
